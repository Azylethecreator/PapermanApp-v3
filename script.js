    ...
    if (debugInfo) {
        ...
        debugInfo.innerHTML = `
            <div style="line-height: 1.8;">
                <strong>Launch Date:</strong> ${LAUNCH_DATE.toLocaleDateString()}<br>
                <strong>Days Since Launch:</strong> ${days}<br>
                <strong>Base Range:</strong> ${baseRange.min} - ${baseRange.max}<br>
                <strong>Current Time:</strong> ${timeString}<br>
                <strong>Hour:</strong> ${hour}:00<br>
                <strong>Time Multiplier:</strong> ${(timeMultiplier * 100).toFixed(0)}%<br>
                <strong>Current Count:</strong> ${currentOnlineCount.toLocaleString()}<br>
                <strong>Next Update:</strong> <span id="countdown">30</span>s
            </div>
        `;
    }
}

// Countdown timer for debug panel
...
// NEW: Past Paper Content Loading System
function generatePastPaperContent() {
    try {
        // Create function name based on selections
        const subjectKey = selectedSubject.toLowerCase().replace(/\s+/g, '');
        const functionName = `generateGrade${selectedGrade}${selectedSubject.replace(/\s+/g, '')}Paper${selectedPaper}_${selectedPastPaperYear}_${selectedSession}`;
        
        console.log('Looking for past paper function:', functionName);
        
        // Try to find the function in global scope
        if (typeof window[functionName] === 'function') {
            console.log('Found past paper function:', functionName);
            return window[functionName]();
        }
        
        // Fallback: try basic function name
        const basicFunctionName = `generateGrade${selectedGrade}${selectedSubject.replace(/\s+/g, '')}Paper${selectedPaper}`;
        if (typeof window[basicFunctionName] === 'function') {
            console.log('Found basic past paper function:', basicFunctionName);
            return window[basicFunctionName]();
        }
        
        console.log('No past paper function found, using default content');
        
        // Default content if no specific function found
        const examInfo = `${selectedPastPaperYear} ${selectedSession} Examination`;
        
        return `
            <section class="note-section">
                <p class="lead-text">Past Paper: Grade ${selectedGrade} ${selectedSubject} - ${examInfo} - Paper ${selectedPaper}</p>
                <div class="exam-info">
                    <strong>Status:</strong> Content coming soon! &nbsp;&nbsp; <strong>Note:</strong> This paper will be added in the next update.
                </div>
            </section>
            
            <section class="note-section">
                <h2>Past Paper Information</h2>
                <p>We're working on adding this specific past paper to our collection. In the meantime, you can:</p>
                
                <div class="key-points">
                    <div class="point">Try other available papers for ${selectedSubject}</div>
                    <div class="point">Use our Paperman Quiz feature to practice questions</div>
                    <div class="point">Review study notes for this subject</div>
                </div>
            </section>
            ...
            <section class="note-section">
                ...
                <div class="organelle-grid">
                    <div class="organelle-card">
                        <h3>Full Paper</h3>
                        <p>Complete examination with all questions</p>
                    </div>
                    <div class="organelle-card">
                        <h3>Interactive Memo</h3>
                        <p>Step-by-step solutions with reveal buttons</p>
                    </div>
                    <div class="organelle-card">
                        <h3>Mark Schemes</h3>
                        <p>Detailed marking guidelines</p>
                    </div>
                </div>
            </section>
        `;
    } catch (error) {
        console.error('Error generating past paper content:', error);
        return `
            <section class="note-section">
                <p class="lead-text">Error loading past paper content. Please try again.</p>
            </section>
        `;
    }
}
...
// Updated: Select past paper year now goes to session selection
...
// NEW: Show past paper content (reusing notes container)
function showPastPaperContent() {
    // Generate content using new system
    const content = generatePastPaperContent();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Show debug panel if in dev mode
    if (DEV_MODE) {
